...
void main() {
  group('signing.operations.ChangeKeyOperation', () {
    ...
    test('can be decode a signed operation', () {
      ChangeKeyOperation decoded = RawOperationCoder.decodeFromBytes(PDUtil.hexToBytes(fixture['raw']));

      expect(PDUtil.byteToHex(PDUtil.encodeBigInt(decoded.signature.r)), fixture['r']);
      expect(PDUtil.byteToHex(PDUtil.encodeBigInt(decoded.signature.s)), fixture['s']);
      ...
      expect(PublicKeyCoder().encodeToBase58(decoded.newPublicKey), fixture['newPublicKey']);
    });

    test('can be decode signed operation and encode it again', () {
      ChangeKeyOperation decoded = RawOperationCoder.decodeFromBytes(PDUtil.hexToBytes(fixture['raw']));

      expect(PDUtil.byteToHex(RawOperationCoder.encodeToBytes(decoded)), fixture['raw']);
    });

    test('can be build by hand', () {
      ChangeKeyOperation op = 
        ChangeKeyOperation(
          signer: AccountNumber.fromInt(fixture['signer']),
          newPublicKey: PublicKeyCoder().decodeFromBase58(fixture['newPublicKey'])
      )
      ...
      ..withSignature(Signature(r: PDUtil.decodeBigInt(PDUtil.hexToBytes(fixture['r'])),
    ...
    test('can be built by hand and signed', () {
      PrivateKey pk = PrivateKeyCoder().decodeFromBytes(PDUtil.hexToBytes('CA02200046D101363B3330D65373A70F6E47BB7745FC8EE1F9B3F71992D6B82648158D73'));
      ChangeKeyOperation op = 
        ChangeKeyOperation(
          signer: AccountNumber.fromInt(fixture['signer']),
          newPublicKey: PublicKeyCoder().decodeFromBase58(fixture['newPublicKey'])
