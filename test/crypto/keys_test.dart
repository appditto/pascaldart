...
void main() {
  group('crypto.keys', () {
      ...
      CURVE_INSTANCES.forEach((curve) {
        if (curve.supported) {
          for (int i = 0; i < 100; i++) {
            ...
            expect(PDUtil.byteToHex(Keys.fromPrivateKey(kp.privateKey).publicKey.ec()), PDUtil.byteToHex(kp.publicKey.ec()));
          }
        }
      });
    });
    ...
    test('can retrieve a keypair from a private key', () {
      fixtures.curve714.forEach((c) {
        if (Curve(714).supported) {
          KeyPair kp = Keys.fromPrivateKey(PrivateKeyCoder().decodeFromBytes(PDUtil.hexToBytes(c['enc_privkey'])));
          expect(kp.curve.id, 714);
          expect(PDUtil.byteToHex(PrivateKeyCoder().encodeToBytes(kp.privateKey)), c['enc_privkey']);
          expect(PublicKeyCoder().encodeToBase58(kp.publicKey), c['b58_pubkey']);
        } else {
          expect(() => Keys.fromPrivateKey(PrivateKeyCoder().decodeFromBytes(PDUtil.hexToBytes(c['enc_privkey']))), throwsException);
        }
      });
    ...
    test('can sign a value', () {
        fixtures.curve714.forEach((c) {
          if (Curve(714).supported) {
            PrivateKey pk = PrivateKeyCoder().decodeFromBytes(PDUtil.hexToBytes(c['enc_privkey']));
            Signature sig = Keys.sign(pk, PDUtil.stringToBytesUtf8('test123'));
            expect(PDUtil.byteToHex(PDUtil.encodeBigInt(sig.r)).length == 64, true);
            expect(PDUtil.byteToHex(PDUtil.encodeBigInt(sig.s)).length == 64, true);
          }    
        });
        fixtures.curve715.forEach((c) {
          if (Curve(715).supported) {
            PrivateKey pk = PrivateKeyCoder().decodeFromBytes(PDUtil.hexToBytes(c['enc_privkey']));
            Signature sig = Keys.sign(pk, PDUtil.stringToBytesUtf8('test123'));
            expect(PDUtil.byteToHex(PDUtil.encodeBigInt(sig.r)).length == 96, true);
            expect(PDUtil.byteToHex(PDUtil.encodeBigInt(sig.s)).length == 96, true);
          }    
        });
        fixtures.curve716.forEach((c) {
          if (Curve(716).supported) {
    ...
    test('can retrieve a keypair from an encrypted private key', () {
      fixtures.curve714.forEach((c) {
        if (Curve(714).supported) {
          PrivateKey pKey = PrivateKeyCrypt.decrypt(PDUtil.hexToBytes(c['encrypted']), c['password']);
