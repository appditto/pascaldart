  ...
  group('crypto.encrypt.pascal.PrivateKeyCryptor', () {   
    ...
    test('can encrypt and decrypt a private key', () {
      fixtures.curve714.forEach((c) {
        PrivateKey pkDecrypted = PrivateKeyCrypt.decrypt(PDUtil.hexToBytes(c['encrypted']), c['password']);
        ...
        expect(PDUtil.byteToHex(PrivateKeyCoder().encodeToBytes(pkDecrypted2)), c['enc_privkey']);        
      });
    });
  });
}
