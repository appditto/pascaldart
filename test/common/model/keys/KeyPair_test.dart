  ...
  group('common.model.keys.KeyPair', () {
    test('can be created from a public and private key', () {
      fixtures.curve714.forEach((c) {
        PrivateKey priv = privCoder.decodeFromBytes(PDUtil.hexToBytes(c['enc_privkey']));
        ...
        expect(PDUtil.byteToHex(privCoder.encodeToBytes(kp.privateKey)), c['enc_privkey']);
    ...
    test('does not allow different curves', () {
      fixtures.curve714.forEach((c) {
        PrivateKey priv = privCoder.decodeFromBytes(PDUtil.hexToBytes(c['enc_privkey']));
        String b58wrong = 'gD8AW4sifppye1utSGPZn2TpTkTXd1EAxGiGTfEEZe87JXCx7oCKFw3ew6hz8E5RB66NuMjSzp59qWu4YrwSEjBh4P9RSsb2Xhc5z7rA4feG5eCAz9mQYqCTQRYsDQHwtftCxMRB625c5X7b4';
      ...
      fixtures.curve715.forEach((c) {
        PrivateKey priv = privCoder.decodeFromBytes(PDUtil.hexToBytes(c['enc_privkey']));
        String b58wrong = '3GhhbopVb9wfo4HzecYwKYMWRvLCssTeFWjocfnWv12Yt3GtaW3seeatH9GqhVmnYrF586RKLwjFFMYn7Txq8X2D4qT7CbqrZgbdRm';
        PublicKey pub = pubCoder.decodeFromBase58(b58wrong);
